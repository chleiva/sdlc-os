...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    ...
    # TODO (flagged in README.md, not resolved here — an operational
    # choice this deliverable does not make unilaterally): narrow this to
    # a CIDR allowlist or private-only + bastion/VPN before real
    # production use. Left matching the pilot's setting (open, so
    # bootstrap.sh can reach the API server from outside the VPC) so this
    # composition is at least apply-consistent with pilot-aws-g7e today.
